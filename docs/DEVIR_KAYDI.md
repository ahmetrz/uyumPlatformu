...
| **Sonraki ölçüm — R-D alan eşlemesi + TR-ENERJI 0.3.0 + R5 PostgreSQL + P7 dağıtım + R3 yedek** (`claude/uyumplatformu05-kod-l8y12k`, PR #44; ölçülen `a7cf2a5`) | `kapi:parti` **TAM (hızlı + yavaş + postgresql + compose) · 24 kapı · geçti 24 · KIRMIZI 0 · ÖLÇÜLMEDİ 0** · 194 dosya · 3 527 vaka · 1 atlandı · senaryo 324 / GAP 0 · **iki sağlayıcıda da tam küme yeşil** (SQLite ve PostgreSQL, atlanan sayısı artmadı) · TR-ENERJI 0.3.0: 8 çerçeve / 3 803 madde (89 aile + 3 691 kontrol + 23 mevzuat) · alan eşleme beyanı 8/8 çerçevede, sabotaj 11/11 · **compose kurulumunda rota duman 60/60 · kusur 0 · sayfa hatası 0 · readiness 2,5–4,4 sn · temizlik iki tanıkla doğrulandı** (aynı ölçüm CI'da `kapi-compose` işinde de yeşil) · R3 tatbikatı KOŞTURULDU: yedek al → `prisma/dev.db` taşındı, `veri/kanit` silindi → geri yükle → `--karsilastir` SAĞLAM → tam küme geri yüklenen veritabanına karşı yeşil; PostgreSQL yolu ayrıca ölçüldü (158 tablo · 0,38 MB döküm · boş veritabanına geri yükleme sonrası içerik özeti aynı). **Kapının kurulumda bulduğu beş kusur** (hiçbiri `next dev`de görünmüyordu): derleme anında veritabanı sorgusu · yutulan ön-render sinyali · oturumsuz detay rotasında 307 yerine 500 · `/sistem`in okuduğu dosya imajda yok · kiracı adı istemcide derleme sabiti (58 sayfada hidrasyon uyuşmazlığı). Bağımsız inceleme (R-B) 21 bulgu (4 P1 · 7 P2 · 10 P3); 20'si kapatıldı, biri R0-15 olarak sahibi ve kapanış aşamasıyla ertelendi |
