...
| R0-17 | Ekranda yazan 51 politika cümlesinin gerçek yol ölçümü YOK | R-F ilk ölçümü (10 Eyl 2026): `app/**` taramasında **72** politika cümlesi türetildi; 8'i iddia değil, **64'ü politika iddiası**; **13'ü** ölçüme bağlıydı, **51'i bağlanmadı**. **51 sınıflandırıldı** (ölçerek, elle değil — `sonucSinifi()` cümleden türetir ve bekçi yeniden türetip kütüktekiyle karşılaştırır): **S1 · yetki ve güvenlik 12** (ihlali veri sızdırır ya da OT ağına paket yollar) · **S2 · ürün değişmezi 23** (ihlali güveni bozar) · **S3 · bilgilendirme 16** (yanıltır, zarar sınırlı). **S1'in TAMAMI bu turda eritildi: 12 → 0** ve sınıf tavanı SIFIRDIR — gerekçeli istisna kabul edilmiyor (`web/tests/bekci/politika-olcumu.test.ts`, URN-POL-001). Eriten iki dosya gerçek yolu sürer: `web/tests/politika-pasif-once.test.ts` ağ ilkellerini FIRLATAN sahteyle değiştirip anlık · sapma · kayıttan anlık · temel onaylama yollarını koşturur (SIS-PAS-001), `web/tests/politika-yetki-kapilari.test.ts` yetkisiz · okuyucu · katkıcı · tesise kısıtlı rollerle GERÇEK sunucu eylemlerini çağırır ve kaydın değişmediğini ölçer (SIS-YTK-010). Sabotajla kanıtlandı: bir S1 satırı `olculmedi`ye çevrilince (S32), sınıf elle S1→S3 indirilince (S33), sınıf tavanı gevşetilince (S34), `yetkiZorunlu` yazmadan okumaya düşürülünce (S35) ve pasif yola bir `fetch` konunca (S36) kapı KIRMIZI yandı. **AYNI PARTİDE TÜRETİCİNİN KÖRLÜĞÜ ÖLÇÜLDÜ** (bağımsız inceleme, #50 tur 1): türetici yalnız `app/` tarıyor, `\w` Türkçe harfi tanımıyor, `+` ile bölünmüş cümleleri parça parça ölçüyor ve `.demo.ts` dosyalarını dışlıyordu. Dördü de düzeltildi; aday **76 → 125**'e çıktı. **Kör bir tavanın düşük olması iyi haber değildir**: ortaya çıkan 23 yeni S1 satırının HEPSİ aynı partide eritildi (`tests/politika-eylem-kapsami.test.ts` · 15 vaka · SIS-YTK-011, demo ikizleri · SIS-DEM-002, `veri-koruma` · KVK-ENV-005) ve **S1 tavanı SIFIRDA kaldı: 44/44 ölçülü**. Cırcıra BEŞİNCİ DİŞ eklendi: tavan yükselmesi `tavanGerekceleri` altında `eski → yeni` olarak anlatılmadan geçmez (sabotaj S43 · S44 kırmızı yaktı). **S2 · ÜRÜN DEĞİŞMEZİ DE ERİTİLDİ: 26 → 0** (10 Eyl 2026, ikinci tur). Yirmi altı cümlenin her biri iddiayı uygulayan kodun GERÇEK YOLUNU süren bir vakaya bağlandı (`web/tests/politika-s2-eylem.test.ts`, 27 vaka, SIS-DGM-001): motorlar gerçekten koşturulur (uygulanabilirlik override'ı, bildirim süresi, veri koruma, topoloji sapması) ve dokunmadığı iddia edilen kaydın DEĞİŞMEDİĞİ ölçülür; sunucu eylemleri gerçekten çağrılır ve reddin yanında YAN ETKİSİZLİK de delta ile ölçülür. Sabotaj turu: on iki sabotajın on biri kırmızı yaktı; **S60 YAKMADI ve bu bir bulgudur** — motor adı şeması gevşetilince eylem yine reddediyordu ama POLİTİKA gereği değil, `ISLER[ad]` `undefined` gelip ÇÖKEREK. Test 'politika reddetti' ile 'kod çöktü'yü ayırt etmiyordu; reddin SEBEBİ ve motorun gerçekten koşmadığı (`IsKosusu` satırı, ayrı tanık) eklendi ve sabotaj yandı. Cırcırın BEŞİNCİ DİŞİNİN kendi kusuru da ölçüldü: son gerekçenin `yeni` değerinin bugünkü tavana EŞİT olmasını istiyordu, yani yükselmeyi değil DEĞİŞMEYİ ölçüyordu — S2 tavanı 26'dan sıfıra indirilince, cırcır tam da istenen yönde sıkılırken kırmızı yandı. Diş TABAN DALA göre yeniden kuruldu: düşüş ve eşitlik gerekçe istemez, yükseliş tam o yükselişi anlatan bir gerekçe ister. **Kalan 39'un tamamı S3'tür** (bilgilendirme); S1 ve S2 tavanları SIFIRDIR. Ayrıca R-F EKİ yazıldı ve ALTINCI DİŞ kuruldu: taban dalda olmayan YENİ bir politika cümlesinin varsayılanı ÖLÇÜLÜ olmaktır; gerekçeli istisna kapanış aşaması ister, S1'de istisna hiç kabul edilmez (sabotaj S50–S54, beşi de kırmızı). **KAPANDI (11 Eyl 2026).** Kalan 39 S3 cümlesi üç mekanizmaya ayrılıp eritildi (demo ikizleri `SIS-DEM-003` · sunucu ret gerekçeleri `SIS-DGM-002` · ekran cümleleri `SIS-EKR-001`) ve cırcıra YEDİNCİ DİŞ eklendi: ölçülmeyen politika cümlesi SIFIRDIR, gerekçe onu açmaz. **Ama asıl bulgu paydaydaydı:** bağımsız inceleme (Brief L · tur 1) türeticinin yalnız TIRNAKLI DİZE sabitlerini taradığını, ekranda duran tırnaksız JSX metnini hiç görmediğini ölçtü; aynı turda sınıf kalıplarındaki ASCII `\b` kusuru da çıktı (Türkçe ekler yüzünden `sır` "sırrının"da, `göremez` "görünmez"de tutmuyordu — POL-118 bir SIR SIZINTISI iddiası olduğu hâlde S3 sayılıyordu). Türetici genişletildi ve kalıplar `(?<![\p{L}])…(?![\p{L}])` ile `u` bayrağına çevrildi: kütük **131 → 184 satır**, `POLITIKA` **123 → 175**. Açılan **52 cümlelik borç** (S1 18 · S2 23 · S3 11) AYNI PARTİDE eritildi; üç yeni ölçüm dosyası yazıldı (`politika-s1-ekran.test.ts` · `SIS-YTK-012` 22 vaka, `politika-s2-degismez.test.ts` · `SIS-DGM-003` 21 vaka, `bekci/olu-bag.test.ts` · `SIS-BAG-001`). Altı satırın sınıfı kütükten değil CÜMLEDEN yeniden türetildi. **Bugün: 184 satır · 175 politika · ölçülmeyen 0 (S1 0 · S2 0 · S3 0).** Sabotaj: S138–S169 · 38 koşum · 32 kırmızı · 6 yakmadı — ikisi TESTİN kusuruydu (S139 `okuyucu` rolü yazma ile onayı ayırt etmiyordu · S159 sağlayıcının rol eşlemesi boştu, "yetkisiz doğar" ölçümü bedavaydı), dördü SABOTAJIN (iki kapılı yollar ve yanlış kod yolu). Kör bir paydanın oranı her zaman iyi görünür: kilidin sayıyı değil KÜMEYİ ölçmesi bu yüzden gerekiyor.  **DÜZELTME TURU (11 Eyl 2026): PAYDA DÖRT KEZ KÖR ÇIKTI.** Gözden geçirenin teşhisi — *"sayaç cırcırı, türeticinin hiç görmediği cümleyi göremez"* — doğru çıktı ve iki kez daha ölçüldü: (3) dize kalıbı AÇGÖZLÜYDÜ ve bir eşleşme sonraki tırnakları YUTUYORDU — tavan 300'den 400'e çıkarılınca POL-062 kütükten SESSİZCE DÜŞTÜ, yani tavanı YÜKSELTMEK popülasyonu KÜÇÜLTÜYORDU; düzenli ifade bir sözcük çözümleyicisiyle değiştirildi (**213 → 215**, çıkan iki cümleden biri R-F'nin doğduğu "Bu kurulumda MFA ZORUNLU." cümlesiydi). (4) Boş duruma SEBEP yazmak yeni politika cümlesi DOĞURDU (**215 → 216**) — R-G ile R-F birbirini besliyor. **Asıl düzeltme MEKANİZMADA:** payda artık tek bir türeticiden gelmiyor. `web/arac/dom-tanik.mjs` kaynağı HİÇ OKUMADAN ürünü gerçek tarayıcıda gezip kullanıcıya görüneni toplar; ayrışma `web/tests/bekci/dom-tanik.test.ts` (URN-TNK-001) ile kırmızıdır. Tanığın ERİŞİM SINIRI da ölçülü ve beyanlı: yalnız sayfanın AÇILIŞ hâlini gezer ve 216 satırın **32'sini** görür — sayı bir taban taşır, "ayrışma 0" tek başına bir şey söylemez. Tanığın ekranda görüp kütükte bulamadığı, çalışma anında BİRLEŞEN sekiz cümle `web/arac/dom-tanik-kutugu.json`da ölçümleriyle durur ve o kütük taban dala göre BÜYÜYEMEZ. Ölçüm tabanı da düzeltildi (inceleme P1-4): `politika.cumle` tabanı 131'de kalmıştı, ölçülen 216'ydı — 85 satırlık sessiz daralma penceresi; politika aracına `--taban-yaz` eklendi. **BUGÜN: 216 satır · POLITIKA 208 (S1 85 · S2 60 · S3 63) · IDDIA_DEGIL 8 · ölçülmeyen 0 · SINIFLANDIRILMADI 0.** Sabotaj: bu turda 45 koşum · 35 kırmızı · 7 yakmadı · 3 zayıf; yakmayanların üçü TESTİN, biri KURALIN (ölü koşul silindi), ikisi SABOTAJIN kusuruydu ve biri YANLIŞ TEŞHİSİMDİ — `kapsamZorunlu` düşen kapsam değerlerini soruya hiç koymuyor, yazdığım yeni kapı ölüydü ve GERİ ALINDI. | **kapandı** — 216 satır · 208 politika · ölçülmeyen 0, sıfırda kilitli |
| R0-18 | Kurulum provası: ürün kuruluyor ve İÇİNE GİRİLEMİYORDU | **Kapandı** (10 Eyl 2026). `docs/KURULUM.md` harfiyen izlenerek temiz bir ortamda kurulum yapıldı: 174 tablo göç etti, sağlık ucu 200 döndü, üç bağımlılık da sağlıklıydı — sonra `/` açıldı ve `/giris`e düştü. `Kullanici` tablosunda **sıfır satır** vardı ve belgede ilk kullanıcıyı kuran hiçbir adım yoktu. Otuz iki kapının hiçbiri bunu göremezdi: `rota:duman` da `kapi-compose` da kuruluma bir kullanıcı KOYARAK ölçüyor. `web/arac/kurucu-hesap.ts` yazıldı — yalnız BOŞ kurulumda çalışır (aksi hâlde kalıcı arka kapı), parola argümandan alınmaz, parola/uzunluğu/özeti denetim izine girmez, kullanıcı+yetki+iz tek transaction. Ölçümü `web/tests/kurucu-hesap.test.ts` (9 vaka) ve GERÇEK kurulumda koşum (2 sn; ikinci koşu reddedildi ve hiçbir şey yazmadı). Test, aracın `tsx` altında hiç açılmadığını da yakaladı (üst düzey `await`). Ayrıntı: `docs/KURULUM_PROVASI.md`. | Kapandı · sahibi KODLAYAN |
| R0-20 | Boş durum sayımı · R-G ilk ölçümü | **Ölçüldü ve büyük ölçüde kapatıldı** (10 Eyl 2026). Kurulum provasının KUSUR 3'ü tekil değil SINIFSALDI: boş durum, sistemin elindeki bilgiyi kullanmıyordu. Boş durumlar R-F yöntemiyle TÜRETİLDİ (`arac/bos-durum-kutugu.mjs`, `<BosIlk>` çağrıları dengeli okunur) ve **74** boş durum bulundu. İki ölçüt de KODDAN türetilir: **(a) NEDEN** — cümle en az iki yan tümceyle sebebini ya da sonucunu söyler; **(b) NE YAPMALIYIM** — `eylem` verilmiştir, tek istisnası İYİ HABER boş durumudur (bayrak bileşenin kendi API'sinden gelir, kütükten değil). İlk ölçüm: **74 boş durum · iki ölçütü karşılayan 43 · nedensiz 20 · eylemsiz 12**; iyi haber ayrımı yapılınca eylemsiz **0**a indi (14 iyi haber durumu eylem istemez) ve karşılayan **54** oldu. **19 nedensiz cümle eritildi** → **73/74**. Kalan tek satır `/regulasyonlar`ın HESAPLANAN cümlesidir (`katalogBoslukCumlesi`): türetici JSX metin sabitini okur, fonksiyondan dönen metni okuyamaz — türeticinin sınırıdır, ekranın kusuru değil; istisna listesinde ölçümünü ADIYLA beyan eder ve ölü referans kabul edilmez. Cırcır: eylemsiz tavanı SIFIR, nedensiz tavanı ölçülene eşit, taban dal karşılaştırması (`web/tests/bekci/bos-durum.test.ts`, SIS-BSD-001). Sabotaj S79–S83 beşi de kırmızı yaktı; S80 ilk yazımda kalıba hiç uymadı ve YENİDEN YAZILDI (R-E: yakmayan sabotaj her zaman testin kusuru değildir — sabotajın kendisi de zayıf olabilir). **Beklenen yan etki:** sebebini söyleyen cümle çoğu zaman bir POLİTİKA iddiası da taşır; R-F türeticisi beşini yakaladı ve ALTINCI DİŞ kendi kuralını uyguladı — beşi de aynı partide ölçüme bağlandı (`tests/politika-bos-durum-iddialari.test.ts`, SIS-BSD-002). Kural CLAUDE.md'ye R-G olarak yazıldı. **TÜRETİCİNİN KÖRLÜĞÜ AYNI PR'DA ÖLÇÜLDÜ** (bağımsız inceleme, #51 tur 1): türetici yalnız `<BosIlk>`e bakıyordu ve evrenin ~%62'sini görmüyordu — `<BosFiltre />` (yirmi beş çağrı) ile satır içi `className="bos"` metinleri (yirmi) kütükte HİÇ YOKTU, üstelik kuralın doğduğu `/tesisler` örneği de oradaydı. "73/74" bu yüzden yanlış bir sayıydı. Üç yüzey de tarandı ve gerçek evren **94** çıktı — **o da yanlıştı**: ikinci tur, karşılaştırma işleci taşıyan bir seçim ifadesinin "hesap" sayılıp satırın TÜMÜYLE düşürüldüğünü ölçtü (envanter ekranının BİRİNCİL boşluğu). Gerçek evren **95**; melt turundan sonra **82/95** (nedensiz **1** — beyanlı hesaplanan cümle; eylemsiz **12** — hepsi `satirIci`). Okunamayan satır artık sessizce düşmez, kütüğe `«okunamadı»` işaretiyle girer. Tavanlar SINIF BAŞINA tutuluyor: tek toplam tavan, sıkı bir sınıfın borcunu gevşek bir sınıfın düzelmesiyle takas ettiriyordu. `BosFiltre` cümlesi bir kez düzeltildi ve yirmi beş çağrı birden kapandı (arketip düzeltmesi); `/tesisler` boşluğuna sebep VE eylem (`/yonetim-tezgahi`) eklendi — R0-19'un `/tesisler` kalemi böylece kapandı. Popülasyon artık `olcum-tabani.json` (`bos.durum: 95`) üzerinden tabanlı; testin içine sabit yazılmış 50'lik taban, arada 44 satırlık sessiz bir daralma penceresi bırakıyordu. **Sahip:** KODLAYAN. **Kapanış:** hesaplanan cümleleri de okuyan bir türetici — ilk müşteri kurulumu öncesi UX turu.  **GÜNCEL ÖLÇÜM (11 Eyl 2026): 100 boş durum · iki ölçütü de karşılayan 99 · nedensiz 1 · EYLEMSİZ 0 · iyi haber 17.** Evren 95'ten 100'e çıktı: türetici DEĞİŞMEDİ, popülasyon değişti — R0-21 kapatılırken ekranlara eklenen yeni boşluklar kütüğe girdi. Nedensiz kalan tek satır hesaplanan bir cümledir (`katalogBoslukCumlesi`) ve ölçümünü ADIYLA beyan ederek istisna listesinde durur; kapanış aşaması P3 · mesaj kataloğudur.  **DÜZELTME TURU (11 Eyl 2026): TÜRETİCİNİN ALTINCI KÖRLÜĞÜ.** DOM tanığı `/zimmetlerim`de HEM sebepsiz HEM eylemsiz bir boş durum gördü; kaynak türeticisi aynı anda "eylemsiz 0 · nedensiz 1" diyordu. Sebep: `cumleMetni` bir ÜÇLÜ İFADENİN bütün dallarını tek metne birleştiriyor ve sebebini söyleyen dal, söylemeyeni ÖRTÜYORDU. Bugün her dal AYRI ölçülür (`cumleDallari`; koşul dalı — `?` işlecinden önceki parça — dal sayılmaz) ve açılan borç aynı partide eritildi: **14 ekranda** sebep cümlesi eksikti. Tanığın kendi seçicisi de kusurluydu ve iki yönde birden: `[class*="bos"]` alt dize eşleşmesi `ab-dok-bosluk` gibi YERLEŞİM sınıflarını yakalıyor, ürünün asıl bileşenini (`BosIlk` — `bos` sınıfı BASMAZ) hiç göremiyordu; ürünün kendi sözleşmesiyle değiştirildi (5 → 16 boş durum). **DÜZELTME TURU · TUR 2 (11 Eyl 2026): YEDİNCİ VE SEKİZİNCİ KÖRLÜK.** (a) `iyiHaber` bayrağı VARLIĞIYLA okunuyordu (`/\biyiHaber\b/`) — `iyiHaber={kosul}` yazan iki ekran, koşul çalışma anında `false` olsa bile muafiyeti alıyordu; bugün yalnız çıplak öznitelik ya da sabit `{true}` muaf sayılır (`iyiHaberMi`) ve açılan üç eylemsiz satır aynı partide kapandı. (b) Paylaşılan `VeriTablosu` KENDİ boş durumunu yazıyordu: sebebini bilmediği bir boşluğa cümle uyduruyor ve hiçbir çağıranın DOLDURMADIĞI bir `bosEylem` yuvası taşıyordu — doldurulmayan bir uzantı noktası sınır değil, üstünde yorum olan eylemsiz bir boş durumdur. Bileşen artık ya ürünün arketipini (`BosFiltre`) çizer ya HİÇBİR ŞEY; popülasyon 126 → 125'e indi ve düşüş gerekçesiyle `olcum-tabani.json`a işlendi. (c) DOM tanığı boş durumları METNE göre anahtarlıyor ve İLK GELENİ saklıyordu: aynı cümle iki ekranda görünüyorsa ikincideki EYLEMSİZ hâl kütüğe hiç girmiyordu — ve cırcırın tuttuğu sayı tam olarak 'eylemsiz 0'dı; bugün EN KÖTÜ hâl kazanır ve bütün rotalar kaydedilir. **BUGÜN: 125 boş durum · iki ölçütü de karşılayan 124 · nedensiz 1 · eylemsiz 0 · iyi haber 17** (`BosIlk` 74 · `BosFiltre` 1 · `satirIci` 48 · `kosullu` 2). | açık — 124/125, eylemsiz 0 (sıfırda kilitli), nedensiz 1 beyanlı |
| R0-21 | Dolu ekranın İÇİNDEKİ bölüm notları eyleme işaret etmiyor (12 satır) | Bağımsız incelemenin (#51 tur 1) açtığı gerçek popülasyonda, satır içi `className="bos"` yüzeyinin **on iki** satırı (tur 2'de türeticinin körlüğü kapatılınca on birden on ikiye çıktı — yeni kusur değil, ölçülmemiş kusurun görünür olması) NEDEN ölçütünü geçiyor ama NE YAPMALIYIM ölçütünü geçmiyor: `envanter/Durus.tsx` (3) · `envanter/EnvanterIstemci.tsx` (4) · `envanter/Yonetisim.tsx` (2) · `tesisler/[id]/Tesis360.tsx` (1) · `raporlar/karne/Karne.tsx` (1). Bunlar EKRANIN YERİNE GEÇEN bir boşluk değil, dolu bir ekranın içindeki bölüm notlarıdır; her birine bir gezinme bağı koymak kullanıcıyı çalıştığı ekrandan koparır ve etkileşim sadeleştirmenin "minimum gereksiz gezinme" ölçütünü ihlal eder. Hangi notun gerçekten bir bağ hak ettiği ekran ekran bir TASARIM kararıdır. **İlk kurulum boşluğunda (`BosIlk` · `BosFiltre`) eylemsiz tavanı SIFIRDIR ve öyle kaldı**; bu satırlar ayrı sınıfın tavanındadır ve `web/arac/bos-durumlar.json` → `tavanGerekceleri` altında sahibi ve kapanış aşamasıyla beyanlıdır. **KAPANDI (11 Eyl 2026).** On ikisinin de çıkışı verildi ve çözüm ARKETİPİK: çözüm aynı ekrandaysa eylem YERİNDEDİR (var olan formu açar ya da listeden ilk kaydı seçer) — gezinme bağı kullanıcıyı çalıştığı ekrandan koparırdı; başka ekrandaysa bağ BAĞLAMI TAŞIR (`?varlik=` · `?tesis=` · `?katalog=`); yetkisi olmayana ölü eylem gösterilmez. `KayitYok` dört yerde kullanılıyor (firmware · yama · korelasyon · SBOM) ve dördünde de sebep aynı — arketip bir kez düzeltildi, dört yer birden kapandı. **Kural yetmedi, kapı gerekti:** "eylemsiz boş durum yasak" kuralı, eylemi OLMAYAN bir boşluğa SAHTE EYLEM uydurma baskısı yaratıyor ve on iki çıkışın ÜÇÜ olmayan bir rotaya gidiyordu (`/entegrasyonlar` ×2 · `/varliklar/<id>`); kodun yanındaki yorum "kullanıcıyı çözüme götürür" diyordu, kullanıcı 404 görüyordu. Hiçbir kapı bunu göremezdi — `rota:duman` envanterdeki rotaları gezer, ekranların İÇİNDEKİ bağları değil. `web/tests/bekci/olu-bag.test.ts` (SIS-BAG-001) yazıldı. **Bugün: 100 boş durum · iki ölçütü de karşılayan 99 · nedensiz 1 (beyanlı hesaplanan cümle) · EYLEMSİZ 0 · iyi haber 17**; sıfır kilidi ÜÇ sınıfa da uygulandı ve `CLAUDE.md`ye yazıldı (R-G eki) — `satirIci` için yazılı olan "sahibi ile kapanış aşaması" yolu artık tarihtir.  **DÜZELTME TURU (11 Eyl 2026): ÖLÜ BAĞ BEKÇİSİ AĞACA DAYANDI.** Bekçinin ilk yazımının ÜÇ kusuru vardı ve üçü de bu deponun adı konmuş başarısızlık biçimiydi: (1) popülasyon ÜRETİLMİŞ BİR LİSTEDEN geliyordu (`arac/rotalar.json`) — üreticisi kör kalırsa bekçi olmayan bir rotayı "var" sayar; bugün desen kümesi `app/` AĞACINDAN türetilir (65 desen, altısı dinamik, rota grubu yola sızmıyor). (2) ÇÖZÜMLEYİCİ GEVŞEKTİ: herhangi bir ÜST yolun bulunması yetiyordu ve `/tesisler/cm1/olmayan/derin` ÇÖZÜLÜYORDU; bugün eşleşme segment segmenttir ve segment SAYISI uyuşmak zorundadır. (3) KAPSAM DARDI: yalnız `href="..."`, yalnız `app` + `components`; bugün beş hedef türü (`href` düz/ifade · `router.push` · `redirect` · `revalidatePath`) ve üç kök — taranan bağ **20 → 423** ve DÖRT ölü hedef daha çıktı: `revalidatePath('/maddeler')`, olmayan bir rotayı tazeleyen sessiz no-op (madde yazıldıktan sonra hedef ekran bayat kalıyordu). DOM tanığının bulduğu bir ölü bağ daha kapandı: `saglik/Cekmeceler.tsx → /isler`. Sabotaj: 7 koşum · 6 kırmızı · 1 yakmadı — yakmayan, TESTİN değil KURALIN kusurunu gösterdi (`parca.length > 0` koşuluna hiçbir girdi ulaşamıyordu) ve ölü koşul SİLİNDİ. | **kapandı** — eylemsiz 0, sıfırda kilitli |
| R0-22 | "Demo sürümü: değişiklikler kaydedilmez" iddiasının kapsamı 4/56 | Bağımsız inceleme şüphesi (#51 tur 1): politika kütüğü CÜMLEYE göre anahtarlıdır, POL-098 tek satırdır ve tek bir demo ikizini sürmek onu "ölçüldü" yapıyordu. Depoda **56** `.demo.ts` var; gerçek yolla sürülen **5** (sayım ikinci turda İÇE AKTARIMDAN türetildi; ad geçişine bakan ilk sayım hem kandırılabiliyordu hem de bir ikizi kaçırıyordu). Yani iddianın %91'i ölçüsüz ve BU SAYI HİÇBİR YERDE YAZMIYORDU — "bilinmeyen ≠ sıfır" kuralının ihlali. Sayı bugün her koşumda YAZILIR ve bir TAVAN tutar: ölçülen ikiz sayısı düşerse kapı kırmızı yanar (`tests/politika-s2-eylem.test.ts` · POL-098 kapsam vakası). **KAPANDI (11 Eyl 2026).** `web/tests/politika-s3-demo-ikizleri.test.ts` (SIS-DEM-003) bütün ikizleri TEK tabloyla sürüyor: **55/55 ikiz · 216 ihraç çağrıldı · 211 ret**. Popülasyon İKİ BAĞIMSIZ yoldan türetilir (dosya sistemi + `'use server'` ölçütü · Vite'ın `import.meta.glob` taraması) ve ikisinin aynı kümeyi verdiği ayrıca ölçülür — tek yola bakan bir sayım, o yol bozulunca sessizce boşalırdı. Dört diş: eşleme kapsamı · ihraç paritesi (eksik ad `undefined is not a function` demektir, yani ekran "Demo sürümü…" DEMEZ, ÇÖKER) · sessiz başarı yok · yapısal olarak yazamaz. **Bulgu:** `kapsamMesaji.demo.ts` ölü ikizdi — gerçek modül `'use server'` değil `server-only` olduğu için takma ad listesine hiç girmiyor, tüketicileri de onu GÖRELİ yolla alıyordu; dosya "demo davranışı burada" diye okunuyordu, oysa demo o kodu hiç çalıştırmıyordu. Silindi. Eski sayaç (test dosyalarında literal içe aktarım arayan) dinamik süpürmeyi göremiyordu ve "5/55" diye YANLIŞ bir sayı basacaktı. | **kapandı** — 55/55, iki yönlü diş |
| R0-23 | Politika ÖZNE kalıbı ÇEKİMLİ gövdeleri görmüyor (beyanlı sınır) | Bağımsız inceleme (düzeltme turu · tur 2) `OZNE` kalıbının ASCII `\b` kullandığını ölçtü: `\b` Türkçe harfle BAŞLAYAN bir alternatifte sözcük sınırı kurmaz — `/\bürün\b/iu.test('Ürün bu engeli aşmaz')` **false** döner. İçlerinde CLAUDE.md'nin S1'i TANIMLARKEN kullandığı arketip de vardı ('Bu ürün OT ağında aktif tarama YAPMAZ'), yani 'S1 tavanı 0 · yedinci diş' kilitlerinin hepsi o cümlenin ÜSTÜNDEN atlıyordu. Sınır Türkçe harf sınırına çevrildi: **216 → 220** satır. İncelemeci ayrıca gövdeleri KÖK hâline getirmeyi önerdi (`kütük|kütüğ`, `kayıt|kayd`, ardından serbest ek); ÖLÇÜLDÜ ve **BU TURDA YAPILMADI**: gövde kalıbı ÖLÇÜLDÜ ve **51** cümle bu yüzden görülmüyor (ilk turda anılan "105" daha gevşek bir denemenin sayısıydı; geçerli olan ölçülendir: `korGovdeSayisi()`), yani 51 satırlık yeni bir popülasyon açıyor ve bu kütüğün yedinci dişi SIFIRDA KİLİTLİ — her satır gerçek yol ölçümüyle gelmek zorunda. Yüz satırı bir turda aceleyle ölçmek, bu deponun kaçındığı şeyin ta kendisidir. Sınırın BEKÇİSİ türetici değil TANIKTIR: çekimli bir özne taşıyan cümle gerçekten ekrana çıkıyorsa DOM tanığı onu görür ve kütükte bulamayınca `web/tests/bekci/dom-tanik.test.ts` KIRMIZI yanar. **Sahip:** KODLAYAN. **Kapanış:** P3 · mesaj kataloğu — arayüz metinleri sözlük anahtarına geçtiğinde özne kalıbı metin taramasından ANAHTAR taramasına döner ve gövde sorunu ortadan kalkar. **DONDURULDU (Brief M · FAZ 3, 12 Eyl 2026).** Sınır artık yalnız beyanlı değil ÖLÇÜLÜ ve KİLİTLİ. *(TARİHSEL — 12 Eyl 2026'da GEÇERSİZ KALDI: ilk dondurma `tavanlar.korGovde` adında bir ALT KÜME tavanı da tutuyordu ve "çekimli özneyle yazılan yeni bir cümle kapıyı yakar" diyordu. O alan KALDIRILDI; geri konursa kapı kırmızı yanar. Sebebi aşağıda.)* İkinci bir diş sayının SIFIR olmadığını ölçer — sıfırsa ya sınır gerçekten kapandı ya da ölçüm bozuldu, ikisi de bakılmadan geçilemez (`web/tests/bekci/politika-olcumu.test.ts` · R0-23). **KİLİT SAYI TEKTİR ve ADAY POPÜLASYONUN TAMAMIDIR: 357** (`tavanlar.korToplam`). Üç sayı uzlaştırıldı ve üçü de AYNI yürüyüşten türetildi: **357** aday popülasyon (25–400 karakter · en az dört sözcük · JSX/biçem parçası değil · yüklem var · yalın özne yok) — kilitlenen sayı; **51** bu 357'nin çekimli gövde taşıyan ALT KÜMESİ (`korGovdeSayisi()`) — kalıbı gövdeye çevirmenin fiyatı, BİLGİ, tavan DEĞİL; **382** aynı yürüyüşün aday süzgeci `politikaMi()` ile hizalanmadan önceki ESKİ hâli — aradaki 25 satır politika cümlesi olamaz (ithal yolu · JSX parçası · dört sözcükten kısa dize), ölçüldü 357 + 25 = 382. **Alt küme neden tavan değil:** alt kümeyi dondurmak üst kümeyi serbest bırakır (P2-14'ün bulduğu delik), tersi ise kapı değil MAYIN olur — üst küme sabitken alt küme büyüyebilir, çünkü kör bir cümleyi çekimli özneyle yeniden yazmak körlüğü hiç değiştirmez ama alt kümeyi bir artırır ve temiz dalı kırmızı yakardı; üst küme kilitli olduğu için alt küme zaten onu aşamaz (51 ≤ 357) ve kapsam kaybı yoktur. Kararın kendisi de ölçülür: alt kümenin SAYI TAVANI (`tavanlar.korGovde`) geri gelirse kapı kırmızı yanar — yasaklanan şey o MAYIN, alt kümeyi izlemenin kendisi değil; kimlik karşılaştıran (sayı değil) bir izleme başka bir mekanizmadır ve bu dişin kapsamında değildir. Tavan taban dala göre de kilitlidir, yükselmesi gerekçe ister ve TABAN OKUNAMAZSA CI'da kırmızıdır. **Sayı tavanının kabul edilmiş sınırı:** bire bir TAKAS görünmez — bir kör aday silinip yerine başka bir kör aday eklenirse toplam 357'de kalır ve kapı susar. Bu her SAYI tavanının doğasıdır (`korToplam` için de böyleydi), bu turda açılmadı ve beyanlıdır; kısmî ikinci bekçi DOM tanığıdır, tanığın ulaşamadığı yüzeyler `docs/OLCUM_DONDURMA.md` (b)-2'de kategorisiyle yazılıdır. Sabotaj: aday popülasyona kör bir satır eklendi, 357 → 358, kapı kırmızı yandı; karşı tanık, alt küme değişip aday sayısı sabit kalınca kapının YANMADIĞINI gösterdi. Kapanışı bir MÜŞTERİ ihtiyacı doğurursa açılır. | **dondu** — tek kilit sayı **357**, taban dal cırcırında, sabotajlı |
| R0-24 | Brief M incelemesinin ERTELENEN bulguları (dondurma kuralıyla) | Bağımsız inceleme (Brief M · tek tur, 25 bulgu) — çoğu aynı partide kapandı (P1-1 kuyruklu boş tablo · P1-2 sunucu kimliği · P1-3 boş öncül · P2-1 dişin gerçek popülasyonu · P2-2 ölü tarama · P2-3 aklayan cümlenin görünürlüğü · P2-4 kapsam genişliği · P2-5 tautoloji · P2-6 dinamik rota eşlemesi · P2-7 taban yazıcısı · P2-8 mayın diş · P2-9 boş koşumda atlanan · P2-13 sessiz geri düşüş · P2-14/P3-8 sınırın gerçek büyüklüğü · P3-1 · P3-2 · P3-5 · P3-6 · P3-7). **ERTELENENLER ve sebepleri:** (a) **P2-10** — `dom-tanik-bos.json` işlenmiş bir kanıt dosyası ve tazelik denetimi yok; CI her koşumda yeniden ÜRETİYOR (`tanik:dom-bos` adımı bekçiden ÖNCE koşar), yani elle düzenlenmiş bir dosya CI'da hiçbir şey kazandırmaz — yerel koşumda kazandırabilir ve bu kabul edilmiş bir sınırdır. (b) **P2-11** — dal çıkarımı kartezyen çarpım almıyor; iki seçim ifadesi birlikte değiştiğinde o varyant hiç üretilmiyor. Üstel büyümeyi göze almak ölçüm aletini büyütmektir. (c) **P2-12** — `&&` kısa devresi ve boş dize dalı varyant üretmiyor; "koşul yanlış → yan tümce çıkmıyor" hâli popülasyona girmiyor. (d) **P3-3** — göç fikstürü kalıbı üçüncü kez yazıldı (`goc-uygula.mjs` aynısını yapıyor); tek nüshaya indirmek bir yeniden düzenlemedir. (e) **P3-4** — `components/**` kategorisi bugün boş kümede. (f) **Codex · dal çıkarımının KARTEZYEN çarpımı** — iki bağımsız seçim ifadesi birlikte değiştiğinde o varyant üretilmiyor (P2-11 ile aynı sınıf, bağımsız olarak ikinci kez bulundu). (g) **Codex · boş/render-olmayan dal** — `Kayıt yok{filtre ? ' — …' : ''}` kalıbında boş dal varyanta girmiyor, yani süzgeçsiz hâlin kuyruksuz metni hiç ölçülmüyor (P2-12 ile aynı sınıf). (h) **Brief M-SON (#55) raporu §7** — DOM tanık kütüğünde veriye bağlı sayı taşıyan cümle (`/aktivite` · "9 kaydın aktörü bilinmiyor"). Tohum değişince yan tümce kayboluyor; **tohuma bağımlı bir taban, taban değildir.** *Sahibi:* KODLAYAN. *Kapanış koşulu:* bir müşteri ihtiyacı ya da bu satırın bir kapıyı yanlış yaktığı/yakmadığı ölçülen bir vaka. **Sekizi de ALETİ ölçen bulgulardır ve `docs/OLCUM_DONDURMA.md` (c) kuralı gereği o turda eritilmez:** ölçüm tarafında yeni iş, bir MÜŞTERİ ihtiyacı ya da bir ÜRÜN kusuru göstermeden açılmaz. **Sahip:** KODLAYAN. **Kapanış:** bir müşteri ihtiyacı ya da ürün kusuru gösterirse. | açık — sekizi de beyanlı, hiçbiri ürün kusuru değil |
| R0-19 | Kurulum provasında kapanmayan üç kalem | `docs/KURULUM_PROVASI.md` §4'te ölçülmedi olarak işaretli: (a) uygulama rolünün tablo sahibinden ayrılması (`KURULUM.md` §4b) bu kurulumda koşturulmadı; (b) kapalı ağda `docker save`/`load` denenmedi; (c) ikinci kullanıcı + rol ataması ve ona bağlı kapsam öğesi/tesis adımı ölçülemedi — sürücü betiğinin seçici kusuru (ürün kusuru DEĞİL, ayrıca doğrulandı). Ek olarak `/tesisler` boş durumu tesisin nerede açıldığını (`/yonetim-tezgahi`) SÖYLEMİYORDU — **kapandı** (#51 tur 1): boşluk cümlesi sebebini söylüyor ve `/yonetim-tezgahi`ne bağ taşıyor. **Ölçülmedi ile çalışmıyor aynı şey değildir**: bu satırlar denenmedi, başarısız olmadı. **Sahip:** KODLAYAN. **Kapanış:** (a) ve (b) ilk müşteri kurulumu; (c) ilk müşteri kurulumu ÖNCESİ UX turu. | açık — ölçülmedi, sahipli, aşamalı |
