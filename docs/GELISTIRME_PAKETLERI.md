...
| R0-13 | Üst çubuk dokunmatik bantlarda taşıyor | ÖLÇÜLDÜ (8 Eyl 2026): viewport 768 iken `.ab-ust` 999px, 375 iken 938px — yani çubuk 1024px altında ZATEN kendi içinde kırpılıyor. `.sag` kümesi bilerek hiçbir genişlikte düşmüyor (`kabuk.css`) ve fark nav ile örtüşüyor. Sektör merceği eklenince örtüşme ana gezinme bağının üstüne düştü ve gezinme kapısı kırmızı yandı; mercek 1024 altında gizlendi (seçim `localStorage`da kalır, dokunmatik bantta DEĞİŞTİRİLEMEZ). Bu bir yama, çözüm değil: doğru iş üst çubuğun dar bant yerleşimini yeniden ele almaktır (taşan öğe kümesi ölçülüp sıraya konmalı). **Sahip:** kabuk · UX dilimi. **Kapanış:** üst çubuk responsive yerleşimi ele alındığında. | açık — ölçüldü, yamayla geçildi |
---
