...
| R0-17 | Ekranda yazan 51 politika cümlesinin gerçek yol ölçümü YOK | R-F ilk ölçümü (10 Eyl 2026): `app/**` taramasında **72** politika cümlesi türetildi; 8'i iddia değil (durum etiketi · sayı · boş hâl), **64'ü politika iddiası**. Bunların **13'ü** iddiayı uygulayan kodun gerçek yolunu süren bir vakaya bağlandı, **51'i bağlanmadı**. Bağlanmayanların hepsi kütükte (`web/arac/politika-cumleleri.json`) SAHİBİYLE ve bu satırla duruyor; sayı bir TAVANDIR ve `web/tests/bekci/politika-olcumu.test.ts` yalnız küçülmesine izin verir. Bu turda sıfıra indirilmedi çünkü 51 uçtan uca vaka yazmak partiyi taşırdı ve yarısı yazılmış ölçüm, ölçülmemiş olandan daha kötüdür: kütükte "ölçüldü" der. **Sahip:** KODLAYAN. **Kapanış:** her partide en az beş satır ölçüme bağlanır (tavan yalnız küçülür); R1 partisinde ilk beş satır kapanır. | açık — ölçüldü, ertelendi |
