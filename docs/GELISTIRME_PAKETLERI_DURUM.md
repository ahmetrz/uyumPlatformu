...
| **B1/B2** `URN-KAP` | **Uygulandı (8 Eyl 2026, PR açık).** Uyum zincirinin öznesi `Tesis` → `KapsamOgesi` (tür katalog: `kot-tesis` · `kot-kurum`; öğe `ko-<tesisId>` köprüsüyle); **dokuz** omurga tablosu `kapsamOgesiId` (`SurecKapsami · MaddeDurumu · UygulanabilirlikKarari · Istisna · KanitKapsami · DenetciKapsami · DegerlendirmeAktarimi · UyumAnlik · Yetki`), tesis tabanlı tablolar `tesisId` tutar, ekranlar köprüden sorar. B2: `TesisProfili`'nin sekiz enerji kolonu `SektorOznitelikSemasi` satırı (`rol · grup · secenekler` sütunları), türetilmiş alan yerine kural bileşimi (`{ hepsi: [teiasScadaEms = true, seriHaberlesme != true] }`). **Tek göç** `20260908180000_b1_b2_kapsam_ogesi` (P2 aynı tablolara `kiraciId` ekleyecek). Bekçiler: URN-KAP-001 (omurgada `tesisId` → kırmızı; izin listesi boş, yalnız küçülür), URN-KAP-002 (paket anahtarı çekirdek kolonu → kırmızı), URN-KAP-003 (tohum = göç; K3 sayım eşitliği dosyadan). RBAC ekseni öğe (`izinliKapsamOgesiIdleri`), kendi kanıtıyla. K4: `docs/kanit/faz-b-k4/` — 14 ölçüm, kırmızı 0; enerji Tesis 360 20 alan, su 12, aynı kod. Ölçüm: 154 dosya (+3 630/−1 040) + kapanış; tsc 0 · eslint 0 · 3 292 vaka · tam kapı kümesi 19/19 · yedi sabotaj. | P1 (bitti) | K25 paketle satış (kalıcı kolon kuralı) · K26 özne = kapsam öğesi | P4: kapsam türleri ve öznitelik şeması PAKETTEN yüklenir; kurum öznitelikleri kapsam öğesine | **154 mevcut** (app 39 · lib 34 · tests 63 · prisma 11 · docs 3) + kapanış |
...
| **P4** `URN-PKT` | **İlk dilim uygulandı (9 Eyl 2026, PR açık).** `lib/paket/` VAR: `bicim.ts` (manifest/sözlük/tür/öznitelik/çerçeve/yükümlülük şemaları, CSV), `dogrula.ts` (yedi hata sınıfı; eksik alan · özet uyuşmazlığı · listede olmayan dosya · tekrar kod · sonra gelen üst madde · lisans sınırı), `kur.ts` (doğrulanmamış paket yazılmaz; tek transaction — sonda patlayan kurulum hiçbir satır bırakmaz, ölçüldü; çerçeve TASLAK; köken; kiracı satırı çelişki olarak korunur; kaldırma = arşiv, silme yok). Göç `20260909090000_p4_icerik_paketi` (eklemeli: `IcerikPaketi · IcerikPaketiSurumu`, `Regulasyon.lisansTuru/metinDahil`, `koken · paketSurumId` × 7; `FrameworkSurumu_tekAktif` kısmi indeksi korundu — elle yazıldı). Eylemler `lib/eylemler2/paket.ts` + demo ikizi. İskeletler `paketler/TR-ENERJI` (17 sözlük · 1 tür · 12 öznitelik · EPDK-SGYM 4 bölüm + 18 + 1 geçici · Ek-3 13 aile + 565 kontrol, 57'si seviyesiz "Ek Kontrol") ve `paketler/TR-BANKACILIK` (15 · 4 · 7 · BDDK-BS 4 kısım + 7 bölüm + 47 madde numarası) — madde METNİ yok; doğrulayıcı 0 hata. Tohum taşınabilirliği ölçüldü, taşınmadı: `docs/P4_TOHUM_TASIMA_OLCUMU.md`. **Kalan:** form/rapor şablonu, rol kataloğu, demo yükleyici, eşleme CSV, `/paketler` ekranı, OSCAL (SCF — hukuki görüş bekleniyor). | P1 (bitti), B1/B2 (bitti) | K18 SHA-256 zorunlu (var), Ed25519 isteğe bağlı (yok) | `DenetimFormuSablonu` + XLSX hücre eşlemesi | 12 mevcut + 15 yeni (ölçülen: lib 4 · eylem 2 · arac 1 · göç 1 · şema · paketler 15 dosya · test 4) |
