...
| `URN-PKT-008` | — | tanımlar yazma yetkili · kurulum (yeniden kurulum) | Sürüm kaydı kurulumdaki içerik özetlerini (`ozetJson`) taşır; paket özetleri yeniden yazılmış ama `surum` aynı · aykiri | Aynı numarayla değişmiş içerik kurulur; aynı numarayla aynı içerik kurulur; yeni numarayla değişmiş içerik kurulur | Değişmiş içerik SÜRÜM hatasıyla reddedilir ("kurulu sürüm değişmez"), sürüm kaydı ve içerik olduğu gibi kalır; aynı içerik idempotent; yeni numara kurulur | Ekran yok | yazma yok (red) | yok | `paket-kur.test.ts` |
