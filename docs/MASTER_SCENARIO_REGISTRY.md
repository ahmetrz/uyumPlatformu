...
| `URN-PKT-007` | — | tanımlar yazma yetkili · kurulum · kaldırma | İz yazıcı transaction istemcisi kabul eder (ortak.ts › iz); kaldırma okuma-karar-yazma tek transaction · aykiri | İz yazımı patlatılır (sentetik) ve paket kurulur / kaldırılır; kök istemciye dokunmayı kaydeden sahte istemciyle kaldırma çağrılır | İz yazılamazsa kurulum da arşiv de GERİ ALINIR: paket kaydı yok / paket kurulu kalır, iz satırı yok; kaldırma aktif sürüm sayımını transaction İÇİNDE yapar — kök istemciye dokunan kaldırma kırmızı | Ekran yok | iz ya durumla birlikte var ya ikisi de yok | yok | `paket-eylem.test.ts` · `paket-eylem.test.ts` · `paket-kur.test.ts` · `paket-kur.test.ts` · `paket-kur.test.ts` |
