...
| `URN-KUR-004` | — | ürünü sürdüren geliştirici · depo geneli | Ad geçici; kalıcı ad sonra verilecek — ad sekiz kod dosyasında düz metindi · çelişen | Kaynak ağacında ürün adını düz metin olarak arar | `lib/marka.ts` dışında hiçbir `.ts`/`.tsx`/`.css` dosyasında ad geçmez; görünen her yüzey `MARKA_AD` okur | Sekme başlığı, kabuk sözcük markası, giriş ve hata ekranı aynı adı gösterir | yazma yok | yok | `marka-adi.test.ts` · `marka-adi.test.ts` |
