...
| §1.1 · §1.2 · §1.3 · §1.4 · §1.5 kurulum | **P4 dilimi (9 Eyl 2026):** `lib/paket/` — `bicim.ts` (şemalar, CSV) · `dogrula.ts` (yedi hata sınıfı) · `kur.ts` (tek transaction, TASLAK sürüm, köken, çelişki raporu, arşiv). Eylemler `lib/eylemler2/paket.ts` (`paketKur` · `paketKaldir`, `tanimlar/yazma`, iz). İskeletler `paketler/TR-ENERJI` · `paketler/TR-BANKACILIK` (`tests/paket-iskeletler.test.ts`). Tohum taşınabilirliği `docs/P4_TOHUM_TASIMA_OLCUMU.md` | demo (§1.9) · eşleme CSV · ekran (`/paketler`) · OSCAL okuyucu; form (§1.6) · rapor (§1.7) · rol (§1.8) katalogları 2.2–2.3 ile geldi — ekranların ve çalışma zamanının bunları okuması kaldı |
