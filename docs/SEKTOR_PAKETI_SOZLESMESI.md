...
| 4 | **Çerçeveler (madde ağacı)** | `cerceve/<kod>.json` — OSCAL katalog + `uzantilar.json` (seviye, zorunluluk tipi, kanıt beklentisi) | **VAR** · `Regulasyon · FrameworkSurumu · SurumFarki · Madde` (hiyerarşi, `zorunlulukTipi`, `kanitBeklentisi`), `MaddeEslestirmesi`; `/ice-aktarim` (admin onay kuyruğu) | OSCAL okuyucu **var** (2.7: `cerceve/<KOD>.oscal.json` CSV'nin alternatifi, aynı kurallar; yazıcı `--oscal`; gidiş-dönüş `tests/paket-oscal.test.ts`); lisans alanı **var** (§2, 9 Eyl 2026); eşleme CSV türü **var** (2.4); **köken sütunları var** (9 Eyl 2026: `kaynak_url · kaynak_yeri · erisim_tarihi · yururluk_tarihi` → `Madde.maddeKaynakUrl · kaynakSayfa · kaynakErisimTarihi · gecerliBaslangic`; kaynağı olduğunu söyleyen çerçevede metinli madde köken taşımak ZORUNDA — KAYNAK sınıfı); **uygulanabilirlik beyanı var** (`uygulanabilirlik: { kapsamTurleri[], kosul?, aciklama? }` → `UygulanabilirlikKurali` köken paket) |
| 5 | **Yükümlülükler ve süreleri** | `yukumlulukler.json` — kod, ad, regülasyon, **tetikleyici** (`olay` · `takvim`), asgari şiddet, süre (saat, **boş bırakılabilir**), **dönem** (`yillik` · `ceyreklik` · `aylik`), **dönem başlangıcı** (`MM-DD`), **teslim günü**, dayanak, merci, kanal notu, alan şablonu | **VAR** · `BildirimYukumlulugu(kod, ad, regulasyonId?, tetikleyici, asgariSiddet, sureSaat?, donem?, donemBaslangici?, teslimGun?, dayanak, merci, kanalNotu?, alanSablonuJson?, aktif)` + `BildirimDonemi` | **TETİKLEYİCİ TÜRÜ ZORUNLU AYRIMDIR (R-D):** olay tetikli yükümlülük `sureSaat` taşır ve `donem` TAŞIYAMAZ; takvim tetikli `donem` taşır ve `sureSaat` TAŞIYAMAZ — doğrulayıcı ikisini de reddeder. Bir takvim yükümlülüğünü olay alanına yazmak biçimi doğru bir anlam eşleme hatasıdır. Süre birimi saat sabit; gün/iş günü paket beyan etmeli (`sureBirimi`) |
| 6 | **Denetim formu şablonları** | `form/<kod>.json` (bölümler · alanlar: anahtar, etiket, tip, seçenek, madde referansı) + isteğe bağlı `form/<kod>.xlsx` ve alan başına `hucre` | **VAR (2.2, 9 Eyl 2026)** · `FormSablonu(kod, ad, tur, sektorId, dosyaAdi, tanimJson, aktif, koken, paketSurumId)`; doğrulayıcı XLSX sayfasını ve hücreleri dosyaya karşı okur, telifli pakette XLSX yasak, kopuk madde referansı kurulumda KİMLİK. **Ekran şablonu OKUYOR (R12, 10 Eyl 2026)**: `/raporlar/denetim-formlari` kurulu ve AKTİF şablonları çekirdeğin iki formunun yanına üçüncü seçenek diye koyar; çekirdek yalnız DOLDURUR (`lib/denetim/sablonDoldurma.ts`) ve dört alan sınıfı vardır — kontrole bağlı ve kurulu (durumu yazılır) · kontrole bağlı ama kurulumda yok (**KUSUR**: “Şablon alanı ürüne bağlanmadı”) · **durum sayımı** (`sayimDurumu`: çekirdeğin durum kodlarından biri; kapsamda o durumdaki kontrol sayısı, kova sayılmadıysa “Değerlendirilmedi” — sayılmamış kova SIFIR DEĞİLDİR) · serbest alan (“Değerlendirilmedi · kurum doldurur”). `sayimDurumu` ile `maddeKod` bir arada olamaz ve `sayimDurumu` alanının tipi `sayi` olmalıdır. Şablonun her alanı ALAN EŞLEME BEYANI ister (§1.10). | XLSX şablonunu üretim tarafında da doldurmak (bugün yalnız JSON yapı doldurulur) |
...
| 10 | **Alan eşleme beyanı** | `manifest.json` → `alanEslemesi: { "<ÇERÇEVE KODU>": [{ kaynakAlan, urunAlani, gerekce }] }` | **VAR (9 Eyl 2026, URN-PKT-022)** · doğrulayıcı `ALAN EŞLEME` sınıfı: temsilî olmayan her çerçeve beyan eder; madde dosyasında DOLU her sütun beyanda geçer; beyanda geçip dosyada karşılığı olmayan alan ölü beyandır; bir ürün alanı iki kez beyan edilemez; temsilî çerçeve (kaynak belgesi yok) beyan edemez; pakette olmayan koda beyan yazılamaz; gerekçe en az 40 karakter. **FORM ŞABLONU DA BEYAN EDER (10 Eyl 2026)**: anahtar form kodudur, `urunAlani` şablonun alan anahtarıdır ve şablonun HER alanı beyanda geçer — serbest alan da (`kaynakAlan: null`; “bu alan kaynakta yok” da bir beyandır). `urunAlani` iki ad uzayı taşıdığı için şema enum'ı gevşedi; yazım hatasını iki diş yakalar: dosyada karşılığı olmayan beyan ÖLÜ BEYAN, beyanı olmayan dolu alan BEYANSIZ | Beyanın DOĞRU olduğunu kapı söyleyemez (§1.10 — kabul edilmiş sınır); ekranda gösterilmesi ve inceleme akışına bağlanması sonraki dilim |
...
### 1.9.1 · Bildirim yükümlülüğünde süre, merci ve kanal (R10)

**`sureSaat` BOŞ BIRAKILABİLİR ve boş, sıfır DEĞİLDİR.** Mevzuat bir süre
belirlememişse ("gecikmeksizin", "en kısa sürede") alan BOŞ gelir; ürün
geri sayım GÖSTERMEZ ve satırda "Süre mevzuatta belirlenmedi" yazar.
`0` yazmak REDDEDİLİR: sıfır saat, doğduğu anda geçmiş bir sayaçtır ve
mevzuatın söylemediği bir şeyi söyler. "Bilinmeyen ≠ sıfır" kuralının
bildirim akışındaki karşılığı budur.

**Merci adı ve kanal notu PAKET DİLİNDEDİR; adres yoktur.** `merci`
mercinin resmî adıdır ("Kişisel Verileri Koruma Kurulu"). `kanalNotu`
insanın nereye bakacağını söyleyen bir CÜMLEDİR ("Kişisel Veri İhlal
Bildirim Formu üzerinden") — **uç nokta, alan adı, IP, port ya da kimlik
bilgisi DEĞİLDİR**. Ürün hiçbir merci sistemine bağlanmaz (§0.4);
bağlanmadığı bir sistemin adresini taşımasının anlamı da yoktur. Kanıt
betiği ekranda `https://` · `env:` · `dosya:` · `vault:` biçimlerinden
hiçbirinin geçmediğini ölçer.

**`alanSablonuJson` mercinin istediği ALANLARI sayar, cevapları değil.**
Taslak metnini ürün YAZMAZ: alanları paket beyan eder, cevapları kurum
doldurur. Boş bırakmak, uydurmaktan iyidir.
...
iskelette `"metin girilmedi"` yazar (`lib/paket/kur.ts`). İki sabiti de
TANIYAN tek yer `maddeMetniDurumu` (`lib/paket/bicim.ts`): ekran metin
yokluğunu metin gibi basmaz — ölçüldü, uyum defteri "lisans nedeniyle
girilmedi" cümlesini bir değerlendirme gerekçesi diye gösteriyordu.
OSCAL yolunda okunmayan `prose`/`props` alanları da telifli çerçevede
LİSANS ile reddedilir (metin veritabanına inmese de PAKETE girerdi) ve
telifli ÇERÇEVE taşıyan paket XLSX form taşıyamaz. Ekran rozeti henüz yok.
...
| §1.1 · §1.2 · §1.3 · §1.4 · §1.5 kurulum | **P4 dilimi (9 Eyl 2026):** `lib/paket/` — `bicim.ts` (şemalar, CSV) · `dogrula.ts` (yedi hata sınıfı) · `kur.ts` (tek transaction, TASLAK sürüm, köken, çelişki raporu, arşiv). Eylemler `lib/eylemler2/paket.ts` (`paketKur` · `paketKaldir`, `tanimlar/yazma`, iz). İskeletler `paketler/TR-ENERJI` · `paketler/TR-BANKACILIK` (`tests/paket-iskeletler.test.ts`). Tohum taşınabilirliği `docs/P4_TOHUM_TASIMA_OLCUMU.md` | demo verisinin JSON'dan yüklenmesi (§1.9, P8); form (§1.6) · rapor (§1.7) · rol (§1.8) katalogları 2.2–2.3, eşleme CSV 2.4, demo paketleri 2.5, `/paketler` 2.6, OSCAL okuyucu 2.7 ile geldi — ekranların ve çalışma zamanının şablon/rolü okuması kaldı |
