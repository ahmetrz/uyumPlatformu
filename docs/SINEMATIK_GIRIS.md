...
`zaman.ts`: poz, ortak kamera ve ekran yerleşimi. `cekirdek.ts`: WebGL sahnesi. `fotograf.ts`: WebGL gerektirmeyen fotoğraf alternatifi. `SinematikGiris.tsx`: yaşam döngüsü, atlama, odak ve DOM geçişi. Görsel dosyaları `public/gorseller/giris/` altındadır; kullanıcının seçtiği sinematik görsellerdir.
