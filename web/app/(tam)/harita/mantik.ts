...
export function yaricap(kuruluGuc: number | null): number {
  if (!kuruluGuc || kuruluGuc <= 0) return 4;
  return Math.min(16, 4 + Math.sqrt(kuruluGuc) * 0.9);
}
...
export function yerlesimKur(satirlar: PortfoySatiri[]): Yerlesim {
  ...
  for (const s of satirlar) {
    ...
    isaretler.push({
      ...
      kuruluGuc: s.kuruluGuc, gucBirim: s.gucBirim, uyumYuzde: s.uyumYuzde,
