...
| `kurucu-hesap.ts` | — (kurulum adımı, `docs/KURULUM.md` §4) | BOŞ bir kurulumun İLK kullanıcısını açar: kullanıcı + küresel yetki + denetim izi TEK transaction. Yalnız boş kurulumda koşar (aksi hâlde kalıcı bir arka kapı olurdu); parolayı ARGÜMANDAN almaz, hiçbir yere yazmaz. Yarışı `count()` değil `Yapilandirma` üzerindeki sabit birincil anahtar kapatır — iki operatör aynı anda koşarsa biri benzersizlik ihlaliyle düşer ve TÜMÜ geri alınır | dolu kurulum · geçersiz girdi · `--parola` argümanı (çıkış 2) |
| `bos-durum-kutugu.mjs` **(CI · bloklayıcı · bekçi üzerinden)** | — (`tests/bekci/bos-durum.test.ts` çağırır) | R-G · boş durum kütüğünü ÜÇ yüzeyden türetir (`<BosIlk>` · `<BosFiltre>` tanımı · satır içi `className="bos"`) ve iki ölçüte vurur: NEDEN söyleniyor mu · NE YAPMALIYIM var mı. `--yaz` kütüğü tazeler, `--taban-yaz --sebep=` popülasyon tabanını indirir | ilk kurulum boşluğunda eylemsiz satır · sınıf tavanını aşan sayı · tabanın altına düşen popülasyon |
| `pg-artik.mjs` **(CI · bloklayıcı · `test:pg` içinde)** | — (kütüphane; elle: `TEST_PG_URL=… node arac/pg-artik.mjs --say\|--supur`) | Sızmış test veritabanlarını süpürür. İKİ diş: sahibinin pid'i canlı mı (ad alanına bağlı) ve veritabanına açık BAĞLANTI var mı (ad alanından bağımsız). `WITH (FORCE)` KULLANMAZ — kullanımdaki veritabanı düşmez, `kalan`a yazılır. Hedef yalnız `TEST_PG_URL`; `PG_URL` kabul edilmez | düşürülemeyen yetim · `psql` yoksa ÖLÇÜLMEDİ (çıkış 1) |
| `taban-dal.mjs` **(CI · bloklayıcı · bekçiler üzerinden)** | — (kütüphane; `bekci/bos-durum` ve `bekci/politika-olcumu` çağırır) | Cırcırların TABAN DAL okumasının ÜÇ HÂLİNİ tek kararda ayırır: dosya tabanda YOK (cırcırın tabanı yok) · OKUNDU · ÖLÇÜLEMEDİ (blob okunamadı · JSON bozuk · biçim nesne değil). Saf fonksiyon, çünkü kütüğü GETİREN bir dalda "dosya var ama okunamadı" hâli ULAŞILABİLİR değildir ve sabotajla sınanamaz | ölçülemedi hâli CI'da KIRMIZI |
