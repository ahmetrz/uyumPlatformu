...
.ab-atla:focus, .ab-atla:focus-visible {`,
  ...
  },
  {
    ...
    dosya: 'app/(kabuk)/(flagship)/Genel.tsx',
    ara: "      title={`${s.ad} · ${kartGucu(s) ?? 'güç ölçülmedi'} · ${skorSozu}${uygunsuz > 0 ? ` · ${uygunsuz} uygunsuz` : ''}`}>",
    yaz: "      title={`${s.ad} · ${kartGucu(s) ?? 'güç ölçülmedi'} · ${skorSozu}`}>",
    testler: ['tests/saha-sadelestirme.test.ts'],
  },
  {
  ...
  },
  {
    ...
    testler: ['tests/saha-sadelestirme.test.ts', 'tests/odak-yayma.test.ts'],
  },
  {
  ...
  },
  {
    ...
    ara: '    aday.set(t.kod, (adSayaci.get(ad) ?? 0) < 2 || !t.sektorAd ? ad : `${ad} · ${t.sektorAd}`);',
    yaz: '    aday.set(t.kod, ad);',
    testler: ['tests/saha-sadelestirme.test.ts'],
  },
  {
    ad: 'Etiket çakışması ikinci geçişte de çözülmüyor',
    kural: 'İki tip hiçbir zaman aynı etiketle çizilmez — sektörü de aynıysa kod ayırır',
    dosya: 'app/(kabuk)/(flagship)/tipEtiketi.ts',
    ara: '  for (const [kod, e] of aday) sonuc.set(kod, (etiketSayaci.get(e) ?? 0) < 2 ? e : `${e} · ${kod}`);',
    yaz: '  for (const [kod, e] of aday) sonuc.set(kod, e);',
    testler: ['tests/saha-sadelestirme.test.ts'],
  },
];
