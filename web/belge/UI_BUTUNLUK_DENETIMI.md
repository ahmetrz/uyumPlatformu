...
|---|---|---|---|---|
| `sektorKaydet` | `lib/eylemler.ts:45` | Evet — Yönetim tezgâhı | `app/(kabuk)/(operasyonel)/yonetim-tezgahi/Formlar.tsx:223` | ERISILEBILIR |
...
| `maddeAlanAta` | `lib/eylemler.ts:168` | Evet — Regülasyonlar | `app/(kabuk)/(operasyonel)/regulasyonlar/Formlar.tsx:107` | ERISILEBILIR |
| `surecKaydet` | `lib/eylemler.ts:183` | Evet — Uyum süreçleri | `app/(kabuk)/(operasyonel)/surecler/Formlar.tsx:82` | ERISILEBILIR |
...
| `bulguGuncelle` | `lib/eylemler.ts:334` | Evet — Bulgu detayı | `app/(kabuk)/(operasyonel)/bulgular/[id]/BulguDetayIstemci.tsx:106` | ERISILEBILIR |
...
| `eslestirmeEkle` | `lib/eylemler.ts:433` | Evet — Eşleştirme | `app/(kabuk)/(operasyonel)/eslestirme/Formlar.tsx:66` | ERISILEBILIR |
| `eslestirmeSil` | `lib/eylemler.ts:452` | Evet — Eşleştirme | `.../eslestirme/Formlar.tsx:103` | ERISILEBILIR |
| `projeKaydet` | `lib/eylemler.ts:463` | Evet — Projeler | `app/(kabuk)/(operasyonel)/projeler/Formlar.tsx:38,104` | ERISILEBILIR |
...
| `kullaniciKaydet` | `lib/eylemler.ts:519` | Evet — Kullanıcı & yetki | `app/(kabuk)/(operasyonel)/yetkiler/Formlar.tsx:40` | ERISILEBILIR |
...
| `aktarimYukle` | `lib/eylemler.ts:570` | Evet — Madde aktarımı | `app/(kabuk)/(operasyonel)/ice-aktarim/IceAktarimIstemci.tsx:90` | ERISILEBILIR |
...
| `ara` | `lib/eylemler2/arama.ts:14` | Evet — Komut paleti (Atlas kabuğu) | `components/KomutPaleti.tsx:4`; kabuğa bağlı: `app/(kabuk)/layout.tsx:15` | ERISILEBILIR |
| `denetimKaydet` | `lib/eylemler2/denetim.ts:41` | Evet — Denetimler | `app/(kabuk)/(operasyonel)/denetimler/Formlar.tsx:79` | ERISILEBILIR |
...
| `kanitTalebiEkle` | `lib/eylemler2/denetim.ts:141` | Evet — Denetimler + Uyum matrisi | `.../denetimler/Formlar.tsx:193`, `app/(kabuk)/(operasyonel)/uyum/UyumIstemci.tsx:397` | ERISILEBILIR |
...
| `varlikKaydet` | `lib/eylemler2/envanter.ts:94` | Evet — Varlıklar | `app/(kabuk)/(operasyonel)/envanter/Formlar.tsx:123` | ERISILEBILIR |
...
| `tumIsleriCalistir` | `lib/eylemler2/isler.ts:32` | Evet — Platform sağlığı | `app/(kabuk)/(operasyonel)/saglik/Eylemler.tsx:34` | ERISILEBILIR |
...
| `kesifEslestir` | `lib/eylemler2/kesif.ts:54` | Evet — Keşif | `app/(kabuk)/(operasyonel)/kesif/Karar.tsx:261` | ERISILEBILIR |
...
| `hesapKaydet` | `lib/eylemler2/kimlik.ts:13` | Evet — Erişim | `app/(kabuk)/(operasyonel)/kimlik/Inceleme.tsx:47` | ERISILEBILIR |
| `erisimIncele` | `lib/eylemler2/kimlik.ts:75` | Evet — Erişim | `.../kimlik/Inceleme.tsx:40` | ERISILEBILIR |
| `etkiOnerisiYenile` | `lib/eylemler2/olay.ts:291` | Evet — Olaylar | `app/(kabuk)/(operasyonel)/olaylar/Eylemler.tsx:198` | ERISILEBILIR |
...
| `degisiklikKaydet` | `lib/eylemler2/operasyon.ts:16` | Evet — Değişiklikler | `app/(kabuk)/(operasyonel)/operasyon/Formlar.tsx:131` | ERISILEBILIR |
...
| `tedarikciKaydet` | `lib/eylemler2/operasyon.ts:214` | Evet — Tedarikçiler | `app/(kabuk)/(operasyonel)/tedarikciler/Eylemler.tsx:46` | ERISILEBILIR |
| `sertifikaKaydet` | `lib/eylemler2/operasyon.ts:259` | Evet — Tedarikçiler | `.../tedarikciler/Eylemler.tsx:114` | ERISILEBILIR |
| `riskKaydet` | `lib/eylemler2/risk.ts:49` | Evet — Risk | `app/(kabuk)/(operasyonel)/riskler/Formlar.tsx:82` | ERISILEBILIR |
...
| `kapsamYenidenHesapla` | `lib/eylemler2/tesis360.ts:88` | Evet — Uyum çerçevesi | `app/(kabuk)/(operasyonel)/uyum/[cerceve]/CerceveIstemci.tsx:354` | ERISILEBILIR |
| `varlikAktarimYukle` | `lib/eylemler2/varlikAktarim.ts:41` | Evet — Varlık aktarımı | `app/(kabuk)/(operasyonel)/varlik-aktarim/VarlikAktarimIstemci.tsx:136` | ERISILEBILIR |
...
| `connectorKaydet` | `lib/eylemler2/entegrasyon.ts:80` | Hayır — Platform sağlığı yalnız okur | `grep` boş; ekran: `app/(kabuk)/(operasyonel)/saglik/page.tsx:4,59` (yalnız `entegrasyonSagligiOzeti`) | UI_EKSIK |
...
| `erisimAta` | `lib/eylemler2/kimlik.ts:53` | Hayır — Erişim ekranı yalnız inceleme sunar | `grep` boş; kardeşleri bağlı: `app/(kabuk)/(operasyonel)/kimlik/Inceleme.tsx:6` | UI_EKSIK |
...
| `yedegiDogrula` | `lib/eylemler2/konfigYedek.ts:47` | Hayır — Yedek & DR ekranı yalnız okur | `grep` boş; ekran importları: `app/(kabuk)/(operasyonel)/yedekleme/YedeklemeIstemci.tsx:12` (yalnız `gorevOlustur`) | UI_EKSIK |
| `sonBilinenIyiIsaretle` | `lib/eylemler2/konfigYedek.ts:89` | Hayır | `grep` boş | UI_EKSIK |
| `yedekBulgusunuIsle` | `lib/eylemler2/konfigYedek.ts:141` | Hayır | `grep` boş; kuyruk ekranda görünür: `app/(kabuk)/(operasyonel)/saglik/page.tsx:57` (`db.veriKalitesiBulgusu.findMany`) | UI_EKSIK |
| `olayGuncelle` | `lib/eylemler2/olay.ts:73` | Hayır — Olaylar ekranında yalnız etki doğrulama var | `grep` boş; ekranın bağladığı üçlü: `app/(kabuk)/(operasyonel)/olaylar/Eylemler.tsx:7` | UI_EKSIK |
...
| `olayKaydet` | `lib/eylemler2/operasyon.ts:117` | Hayır — olay OLUŞTURMANIN hiçbir yolu yok | `grep` boş; `/operasyon` ekranı yalnız `db.degisiklik` okur: `app/(kabuk)/(operasyonel)/operasyon/page.tsx:36` | UI_EKSIK |
| `yedeklemePolitikasiKaydet` | `lib/eylemler2/operasyon.ts:154` | Hayır | `grep` boş; ekran politikayı okur ama yazmaz: `app/(kabuk)/(operasyonel)/yedekleme/page.tsx:35` | UI_EKSIK |
...
| `restoreTestiKaydet` | `lib/eylemler2/operasyon.ts:193` | Hayır | `grep` boş; ekran `GeriYuklemeTesti`'ni kanıt olarak okur (`app/(kabuk)/(operasyonel)/yedekleme/page.tsx:9-16` yorumu) | UI_EKSIK |
| `profilKaydet` | `lib/eylemler2/tesis360.ts:45` | Hayır — Santral 360'ta profil formu yok | `grep` boş; ekran importları: `app/(kabuk)/(flagship)/tesisler/[id]/Plant360.tsx:2-7` | UI_EKSIK |
| `uygulanabilirlikOverride` | `lib/eylemler2/tesis360.ts:100` | Hayır | `grep` boş; kardeşi bağlı: `app/(kabuk)/(operasyonel)/uyum/[cerceve]/CerceveIstemci.tsx:11` | UI_EKSIK |
| `kayittanAnlikAl` | `lib/eylemler2/topoloji.ts:42` | Hayır — /topoloji ekranı iskele | `grep` boş; ekran: `app/(kabuk)/(operasyonel)/topoloji/page.tsx:22` ("BOŞ · YAPIM AŞAMASI") | UI_EKSIK |
...
| Capability | Nerede tanımlı | UI'da erişilebilir mi | Kanıt (dosya:satır) | Sınıf |
|---|---|---|---|---|
| `entegrasyonSagligiOzeti` | `lib/entegrasyon/saglikOzeti.ts:430` | Evet — Platform sağlığı | `app/(kabuk)/(operasyonel)/saglik/page.tsx:4,59`; tipleri de UI'da: `.../saglik/SaglikIstemci.tsx:9` | ERISILEBILIR |
...
| `tesisYedekGorunumu` | `lib/entegrasyon/konfigYedek.ts:404` | Hayır | Tüm repoda çağıran yok (yalnız `konfigYedek.ts:37` yorumu ve `:427` hata metni); /yedekleme ekranı ham `db` sorgusu kullanır: `app/(kabuk)/(operasyonel)/yedekleme/page.tsx:22,27,35` | UI_EKSIK |
...
| `tedarikciOturumOzeti` | `lib/entegrasyon/tedarikciOturum.ts:371` | Hayır | `grep` boş; Tedarikçiler ekranı ham `db` kullanır: `app/(kabuk)/(operasyonel)/tedarikciler/page.tsx:3` | UI_EKSIK |
...
Not: `guvenYazisi` adı `app/(kabuk)/(operasyonel)/kesif/mantik.ts:98`'de **ayrı bir fonksiyon olarak yeniden
...
| `GET /api/v1/assets` | `app/api/v1/assets/route.api.ts:7` → `lib/api/uclar/varliklar.ts` | Hayır (kasten) — karşılığı Varlıklar ekranı | `app/(kabuk)/(operasyonel)/envanter/page.tsx` | MAKINE_ARAYUZU |
...
| `POST /api/v1/assets/observations` | `app/api/v1/assets/observations/route.api.ts:7` → `lib/api/uclar/varlikGozlemleri.ts` | Hayır (kasten) — sonuç Keşif ekranında karara düşer | `app/(kabuk)/(operasyonel)/kesif/Karar.tsx:56` | MAKINE_ARAYUZU |
| `POST /api/v1/vulnerabilities` | `app/api/v1/vulnerabilities/route.api.ts:7` → `lib/api/uclar/zafiyetler.ts` | Hayır (kasten) — zafiyetler Varlıklar/Risk ekranlarında okunur | `app/(kabuk)/(operasyonel)/envanter/mantik.ts` | MAKINE_ARAYUZU |
...
| `POST /api/v1/access-observations` | `app/api/v1/access-observations/route.api.ts:7` → `lib/api/uclar/erisimler.ts` | Hayır (kasten) — karşılığı Erişim ekranı | `app/(kabuk)/(operasyonel)/kimlik/page.tsx:23` | MAKINE_ARAYUZU |
...
`app/(kabuk)/(operasyonel)/topoloji/page.tsx`; `/ice-aktarim` → `app/(kabuk)/(operasyonel)/ice-aktarim/page.tsx`.

### F.2 `app/` altında olup Ray'de olmayan ekranlar (4)

| Rota | Dosya | Rayda mı | Erişilebilir mi | Sınıf |
|---|---|---|---|---|
| `/tesisler` | `app/(kabuk)/(flagship)/tesisler/page.tsx` | Hayır | Evet — derin bağlantı: `app/(kabuk)/(operasyonel)/projeler/ortak.ts:381` (`yol: '/tesisler'`) | ERISILEBILIR |
| `/tesisler/[id]` | `app/(kabuk)/(flagship)/tesisler/[id]/page.tsx` | Hayır | Evet — çok sayıda bağlantı: `app/(tam)/portfoy/Portfoy.tsx:133,173`, `.../uyum/UyumIstemci.tsx:151`, `.../bulgular/BulgularIstemci.tsx:571`, `.../yedekleme/YedeklemeIstemci.tsx:373` | ERISILEBILIR |
| `/sistem` | `app/(kabuk)/(operasyonel)/sistem/page.tsx` | Hayır | Hayır (kasten) — hiçbir `Link` yok; geliştirme ekranı olduğu yazılı: `arac/rota-duman.mjs:20-22` ("RAYDA ÖĞESİ YOKTUR — geliştirme ekranıdır") | MAKINE_ARAYUZU |
| `/sistem/bilesenler` | `app/(kabuk)/(operasyonel)/sistem/bilesenler/page.tsx` | Hayır | Hayır (kasten) — aynı gerekçe; `arac/denetim.mjs:10` bu rotayı otomatik denetimde kullanır | MAKINE_ARAYUZU |
...
`app/(kabuk)/(operasyonel)/topoloji/page.tsx:22` — "BOŞ · YAPIM AŞAMASI". Bu, F.2'deki türden bir raytutarsızlığı
