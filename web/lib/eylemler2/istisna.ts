...
export async function istisnaTalep(girdi: {
  ...
  try {
    ...
    const durum = await db.maddeDurumu.findUniqueOrThrow({
      where: { id: v.maddeDurumuId }, include: { madde: true, kapsamOgesi: { select: { id: true, kod: true, ad: true, tesisId: true } } } });
    if (!izinVar(k, 'uyum', 'yazma', { kapsamOgesiId: durum.kapsamOgesiId, surecId: durum.surecId }))
    ...
    if (acikIstisna) {
      return { ok: false,
        hata: `Bu madde/${await kapsamTerimi(k, 'uyum', durum.kapsamOgesiId)} için `
    ...
    await db.onayTalebi.create({ data: {
      ...
      ozet: `${durum.madde.kod} · ${durum.kapsamOgesi.kod} — ${new Intl.DateTimeFormat('tr-TR').format(bitis)} tarihine kadar istisna: ${v.gerekce.slice(0, 120)}`,
      talepEdenId: k.id } });
