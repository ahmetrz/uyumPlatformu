...
| `manifest.alanEslemesi` | JSON nesne | **alan eşleme beyanı** (9 Eyl 2026, URN-PKT-022): `{ "<ÇERÇEVE KODU>": [{ kaynakAlan, urunAlani, gerekce }] }`. Madde dosyasında DOLU her sütun için bir satır: `kaynakAlan` kaynak belgedeki alanın adı (kaynakta karşılığı yoksa `null` — değeri paket yazarı atadı), `urunAlani` madde sütunlarından biri, `gerekce` (≥ 40 karakter) ürün alanının ANLAMINI anlatır — dönüşümün kolaylığını değil. Beyansız dolu sütun, ölü beyan (dosyada karşılığı olmayan alana beyan), aynı ürün alanına iki beyan, temsilî çerçeveye beyan ve pakette olmayan koda beyan `ALAN EŞLEME` hatasıdır. **Form şablonları da beyan eder** (10 Eyl 2026): anahtar form kodu, `urunAlani` şablonun alan anahtarıdır ve şablonun her alanı beyanda geçer. **Kapı beyanın VARLIĞINI ölçer, doğruluğunu değil** (`docs/SEKTOR_PAKETI_SOZLESMESI.md` §1.10) |
...
kurulamaz.
| `yukumlulukler.json` | JSON dizi | `kod · ad · regulasyonKod? · tetikleyici` (`olay` varsayılan · `takvim`) `· asgariSiddet · sureSaat?` (BOŞ = mevzuat süre belirlememiş; `0` reddedilir) `· donem?` (`yillik`·`ceyreklik`·`aylik`) `· donemBaslangici?` (`MM-DD`; boş = takvim yılı VARSAYILIR ve varsayım beyan edilir) `· teslimGun?` (BOŞ = teslim süresi belirlenmemiş, SAYAÇ YOK) `· dayanak · merci · kanalNotu?` (cümle, ADRES DEĞİL) `· alanSablonu?`. **İki tür karışmaz:** `takvim` tetikli satır `sureSaat` taşıyamaz, `olay` tetikli satır `donem`/`donemBaslangici`/`teslimGun` taşıyamaz — doğrulayıcı ikisini de reddeder (R-D) |
| `form/<KOD>.json` | JSON | denetim formu şablonu: `kod · ad · tur (denetim · oz_degerlendirme · saha) · bolumler[{kod, baslik, alanlar[{anahtar, etiket ≤ 120, tip (metin · sayi · mantik · tarih · secim), secenekler?, maddeKod?, **sayimDurumu?**, zorunlu, hucre?}]}]`; isteğe bağlı `dosya` (aynı dizinde XLSX) + `sayfa`: doğrulayıcı sayfayı ve her alanın hücresini DOSYAYA karşı okur. **Telifli pakette XLSX yasak** (hücre metni denetlenemez). Dosya adı = kod. `sayimDurumu` alanı DURUM SAYIMIDIR: değeri çekirdeğin durum kodlarından biridir (`uyumlu · kismi · uyumsuz · incelemede · degerlendirilmedi · kapsamdisi`) ve ekran o durumdaki kontrol sayısını yazar; `maddeKod` ile bir arada olamaz (biri tek maddeyi, öbürü kümeyi sorar) ve tipi `sayi` olmalıdır. **Şablonun HER alanı manifestte alan eşleme beyanı ister** (§alanEslemesi): anahtar form kodu, `urunAlani` alan anahtarıdır; kaynakta karşılığı olmayan serbest alan `kaynakAlan: null` ile beyan edilir |
...
| Paket | Durum | İçerik |
| --- | --- | --- |
| `TR-ENERJI` | **içerikli** (0.3.0) | sözlük (tohumla birebir), `kontrol_sistemi` türü, 12 öznitelik, `EPDK-SGYM` yönetmeliği (4 bölüm + 18 madde + 1 geçici madde, **tam metin**), **yedi sektör eki** `EPDK-SGYM-EK1…EK7` (89 aile + 3 691 kontrol, **tam metin**; Ek-1 476 · Ek-2 505 · Ek-3 565 · Ek-4 552 · Ek-5 551 · Ek-6 578 · Ek-7 464; metni girilmemiş kontrol 0; "Ek kontrol" kademesi `OPTIONAL`), `EPDK-SGYM-KARNE` rapor şablonu, 2 rol önerisi. Metin EPDK resmî sitesinden (birincil kaynak) 9 Eyl 2026'da alındı; her metinli maddede kaynak adresi, belge içi konum ve erişim tarihi durur. Sekiz çerçevenin hepsi uygulanabilirlik, manifest hepsi için alan eşleme beyan eder. Dayanak: 5846 s. FSEK md. 31 |
